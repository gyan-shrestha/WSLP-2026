...
#SBATCH --mem=96gb
#SBATCH --output=logs/t1gaps_%j.out
...
# The two empty cells in main-text Table 1. Settings copied verbatim from the jobs
# that produced the rest of each grid, so the new numbers are comparable to their
# column: legacy descriptors at 400 handshape codewords for PHOENIX translation,
# spec descriptors for Isharah translation.

echo "########## 1. PHOENIX14T translation, k-center ##########"
conda run --no-capture-output -p $ENV python $PROJECT/src/run_budget.py \
  ...
  --budgets-hours 0.5 1 2 5 --seeds 0 1 2 \
  --n-handshape-clusters 400 --max-epochs 60 \
  --strategies kcenter

echo "########## 2. Isharah translation, articulatory + representativeness ##########"
conda run --no-capture-output -p $ENV python $PROJECT/src/run_budget.py \
  ...
  --archs gloss_free --budgets-hours 7.1 14.2 28.4 --seeds 0 1 2 --max-epochs 60 \
  --strategies pc_art_rep
echo "ALL DONE"
